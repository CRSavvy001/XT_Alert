...
// ---------- CURRENT production path: /twitter/user/last_tweets ----------
// Costs ~135+ credits per call (bills for up to 20 tweets every time),
// but is confirmed working and reliable. Used by the live polling loop
// until the cheaper advanced_search path below is verified.
...
// ---------- EXPERIMENTAL cheaper path: /twitter/tweet/advanced_search ----------
// Only bills for tweets actually returned. Querying "from:<handle>
// since_time:<unix>" should return an empty (near-free) result on the vast
// majority of polls where nothing new has been posted, instead of always
// paying for a full 20-tweet page. NOT wired into the live poller yet —
// test with /debugsearch first and compare the credit cost on the
// twitterapi.io dashboard before switching tick() over to this.
async function fetchAdvancedSearchRaw(handle, sinceUnixSeconds) {
  const query = `from:${handle} since_time:${sinceUnixSeconds}`;
...
// Test command for the cheaper advanced_search path. Defaults to a 1-hour
// window so active accounts should return something to inspect. Check the
// "Credits" column on twitterapi.io's usage dashboard right after running
// this to see the real cost of a call that finds nothing vs. finds tweets.
bot.onText(/\/debugsearch(?:@\w+)?\s+(\S+)(?:\s+(\d+))?/, async (msg, match) => {
  ...
  try {
    ...
    const ms = Date.now() - started;
    const tweets = extractTweets(raw);
    let out = `Response in ${ms}ms.\nstatus=${raw?.status} code=${raw?.code} msg=${raw?.msg || raw?.message}\ntweets returned: ${tweets.length}\n\nCheck twitterapi.io dashboard's Credits column now to see the real cost of this call.`;
    if (tweets.length > 0) {
      const t = tweets[0];
...
bot.onText(/\/start/, (msg) => {
  bot.sendMessage(
    ...
    'Commands:\n' +
      ...
      '/debugapi handle — test the last_tweets endpoint (current production path)\n' +
      '/debugsearch handle [lookbackSeconds] — test the cheaper advanced_search endpoint (experimental, default lookback 3600s)'
  );
});

// ---------- global round-robin polling (still using last_tweets for now) ----------
let queue = [];
let pos = 0;
...
setTimeout(loop, CHECK_GAP_MS);
console.log(`Bot running. Checking one handle every ${CHECK_GAP_MS}ms (no overlap).`);
