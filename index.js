...
bot.onText(/\/debugapi(?:@\w+)?\s+(.+)/, async (msg, match) => {
  ...
  try {
    ...
    if (!tweet) {
      return bot.sendMessage(chatId, `Got a response in ${ms}ms but no tweets found for @${handle}.`);
    }
    bot.sendMessage(
      chatId,
      `OK in ${ms}ms.\nLatest tweet id: ${tweet.id}\nText: ${(tweet.text || '').slice(0, 200)}`
    );
  } catch (err) {
    const ms = Date.now() - started;
...
bot.onText(/\/start/, (msg) => {
  bot.sendMessage(
    ...
    'Commands:\n' +
      ...
      '/debugapi handle — test the API directly for troubleshooting'
  );
});
